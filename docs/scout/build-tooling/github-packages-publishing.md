...
- A failed run can leave a partial snapshot build recorded as latest in GitHub's
  ...
  the safe recovery.
- root `build.gradle.kts` has no publish-retry wrapper and no `githubPublishParallelism`
  property (raptor is on plugin 3.0.0); a durable fix is tracked in the fluid-gradle repo's
  `docs/tasks/github-packages-publish-retry.md` and lands here by upgrading the plugin (planned
  for 4.2.0).
