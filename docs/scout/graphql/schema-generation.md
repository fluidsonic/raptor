...
declaration-order output or a richer default scalar set.

Note: the live code is in `modules/graph/` (package `io.fluidsonic.raptor.graph`).
A near-identical `modules/graphql/` twin exists but its `build.gradle.kts` is marked
`// TODO Deprecated module. Delete.` — anchor against `graph`, not `graphql`.
...
- **The whole schema is alphabetized.** `modules/graph/sources-jvm/graphql/GraphSystemBuilder.kt`
  ...
  argument directives, implemented interfaces, and union possible types. Generated SDL
  field/argument order does **not** match Kotlin declaration order. The sorting is scattered
  across ~8 `build*` methods with no explanatory comment.
- **Only 5 scalars are auto-registered.** `GraphSystemDefinitionBuilder.defaultDefinitions`
  (`modules/graph/sources-jvm/graphql/GraphSystemDefinitionBuilder.kt`) wires in exactly
  `Boolean`, `Double`, `GraphId` (named `"ID"`), `Int`, `String`, before the user's
  definitions. Every other scalar (`Country`, `CountryCode`, `Currency`, `Duration`,
  `LocalDate`, `LocalDateTime`, `LocalTime`, `Locale`, `Timestamp`, `TimeZone`, `Unit`) is an
  opt-in `X.Companion.graphDefinition()` — added individually or in bulk via
  `Definitions.includeDefault()`, which registers `RaptorGraphDefaults.definitions` and is
  idempotent through an `includesDefault` flag (`modules/graph/sources-jvm/assembly/RaptorGraphComponent.kt`).
  Omitting one causes a missing-type error, not a compile error.
