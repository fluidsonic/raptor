...
internal class RaptorServerInstance(
	private val config: RaptorServerConfig
) : RaptorServerScope, DKodeinAware { // FIXME hmm
	...
	override fun beginTransaction(): RaptorServerTransactionImpl { // FIXME here?
		val transaction = RaptorServerTransactionImpl(
			parentScope = this
		)
	...
	private fun Application.configure() {
		...
		install(RaptorTransactionKtorFeature(server = this@RaptorServerInstance))

		config.ktorConfig(this)

		val rootConfig = config.rootRouteConfig
		if (rootConfig != null)
			routing {
	...
	private fun Route.configure(config: RaptorRouteConfig) {
		route(config.path) {
			// FIXME kodein
