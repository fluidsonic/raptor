...
internal class KtorServer(
	...
	val context = DefaultKtorServerContext(parentContext = parentContext)
	...
	private fun Application.configure() {
		...
		val rootConfig = configuration.routingConfiguration
		if (rootConfig != null)
			routing {
	...
	private fun Route.configure(configuration: KtorRouteConfiguration) {
		...
		wrapper {
			route(configuration.path) {
				configuration.customConfig(this)

				intercept(ApplicationCallPipeline.Setup) {
					val parentTransaction = context.attributes.getOrNull(ktorServerTransactionAttributeKey)
						?: return@intercept

					val transaction = parentTransaction.createRouteTransaction(configuration = configuration)
...
val Route.raptorContext: KtorServerContext
